test_that(".find_col returns first matching candidate", {
  ...
  expect_identical(
    zhncommandR:::.find_col(df, c("diagnose", "kodierung")),
    "diagnose"
  )
  expect_identical(
    zhncommandR:::.find_col(df, c("kodierung", "diagnose")),
    "kodierung"
  )
})
...
test_that(".find_col returns NULL when nothing matches (never character(0))", {
  df <- data.frame(a = 1)
  out <- zhncommandR:::.find_col(df, c("b", "c"))
  expect_null(out)
...
test_that(".find_col handles empty candidate vector", {
  df <- data.frame(a = 1)
  expect_null(zhncommandR:::.find_col(df, character(0)))
})

test_that(".n_distinct_nonempty drops NA, empty and whitespace-only", {
  x <- c("AML", "AML", "", " ", NA, "CLL")
  expect_identical(zhncommandR:::.n_distinct_nonempty(x), 2L)
})
...
test_that(".first_nonempty_col coalesces in priority order", {
  ...
  out <- zhncommandR:::.first_nonempty_col(df, c("patient", "patient_2"))
  expect_identical(out, c("A1", "B", "C2"))
})

test_that(".first_nonempty_col handles missing columns and empty data", {
  expect_identical(
    zhncommandR:::.first_nonempty_col(data.frame(a = character()), c("a", "b")),
    character(0)
  )
  expect_identical(
    zhncommandR:::.first_nonempty_col(data.frame(a = 1:3), character(0)),
    rep(NA_character_, 3)
  )
})
