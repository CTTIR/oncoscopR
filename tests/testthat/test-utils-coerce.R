test_that(".as_yesno handles German + English variants and missingness", {
  ...
  expect_identical(zhncommandR:::.as_yesno(x), expected)
})

test_that(".as_yesno passes through logical input", {
  x <- c(TRUE, FALSE, NA)
  expect_identical(zhncommandR:::.as_yesno(x), x)
})
...
test_that(".as_event01 does NOT classify '10 months' as an event", {
  expect_identical(zhncommandR:::.as_event01("10 months"), NA_real_)
  expect_identical(zhncommandR:::.as_event01("100 Tage"), NA_real_)
})

test_that(".as_event01 still maps explicit 1/0 and German tokens", {
  x <- c("1", "0", "ja", "nein", "tod", "lebt", NA, "", "?")
  expect_identical(
    zhncommandR:::.as_event01(x),
...
test_that(".as_event01 maps Dates: present = 1, missing = 0", {
  ...
  expect_identical(zhncommandR:::.as_event01(d), c(1, 0, 1))
})

test_that(".as_event01 date_event mode treats non-empty strings as events", {
  x <- c("2024-01-15", "", NA, "1")
  expect_identical(
    zhncommandR:::.as_event01(x, mode = "date_event"),
    c(1, 0, 0, 1)
  )
})
...
test_that(".safe_date passes Date through and parses character", {
  expect_identical(
    zhncommandR:::.safe_date(as.Date("2024-01-15")),
    as.Date("2024-01-15")
  )
  expect_identical(
    zhncommandR:::.safe_date("2024-01-15"),
    as.Date("2024-01-15")
  )
})
