test_that("onc_normalize_alteration strips trailing Mutation/Mut and collapses ws", {
  expect_identical(
...
test_that("onc_alteration_type classifies the documented categories", {
  ...
  expect_identical(onc_alteration_type(alts), expected)
})

test_that("onc_is_mutation is TRUE only for the Mutation/Variante class", {
  classes <- c("Mutation/Variante", "negativ/kein Nachweis",
               "Strukturell/Zytogenetik: Deletion/Loss")
  expect_identical(onc_is_mutation(classes), c(TRUE, FALSE, FALSE))
})
